...
public class UserControllerApi {
    ...
    //API lấy danh sách
    @GetMapping("/list/{pageNo}/{pageSize}/{sortBy}")

    public ResponseEntity<ResponseObject> getAllUser(@PathVariable Integer pageNo, @PathVariable Integer pageSize, @PathVariable String sortBy){
        //        Page<Users> listUsers = userService.findPaginated(pageNo, pagaSize, sortBy);
        String message = "";
        boolean status = false;
        UserResponse userResponses = new UserResponse();
        if (pageSize > 0 && pageNo > 0) {
            userResponses = userService.findPaginated(pageNo, pageSize, sortBy);
